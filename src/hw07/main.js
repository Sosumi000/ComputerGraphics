...
- Viewing a 3D unit renderedObject at origin with perspective projection
- Rotating the renderedObject by ArcBall interface (by left mouse button dragging)
- Keyboard controls:
...
- Applying Diffuse & Specular reflection using Flat/Smooth shading to the renderedObject
----------------------------------------------------------------------------------*/
...
const renderedObject = new Cone(gl, 32);
...
function setupKeyboardEvents() {
    document.addEventListener('keydown', (event) => {
        if (event.key == 'a') {
            ...
            renderedObject.copyVertexNormalsToNormals();
            renderedObject.updateNormals();
            shadingMode = 'SMOOTH';
            updateText(textOverlay3, "shading mode: " + shadingMode + `(${renderingMode})`);
            render();
        }
        else if (event.key == 'f') {
            renderedObject.copyFaceNormalsToNormals();
            renderedObject.updateNormals();
            shadingMode = 'FLAT';
            updateText(textOverlay3, "shading mode: " + shadingMode + `(${renderingMode})`);
            render();
            ...
            renderedObject.copyVertexNormalsToNormals();
            renderedObject.updateNormals();
            renderedObject.setRenderMode('GOURAUD');
            ...
            renderedObject.copyFaceNormalsToNormals();
            renderedObject.updateNormals();
            renderedObject.setRenderMode('PHONG');
            renderingMode = 'PHONG';
            shader = phongShader;
            updateText(textOverlay3, "shading mode: " + shadingMode + `(${renderingMode})`);
            render();
...
async function initShader() {
    ...
    const vertexShaderSource2 = await readShaderFile('shVertPhong.glsl');
    const fragmentShaderSource2 = await readShaderFile('shFragPhong.glsl');
    phongShader = new Shader(gl, vertexShaderSource2, fragmentShaderSource2);
...
function render() {
    ...
    // drawing the renderedObject
    shader.use();  // using the renderedObject's shader
    shader.setMat4('u_model', modelMatrix);
    shader.setMat4('u_view', viewMatrix);
    shader.setVec3('u_viewPos', cameraPos);
    renderedObject.draw(shader);
